...
int main(int argc, char **argv) {
    int viewPlaneResolutionX = (argc > 1 ? std::stoi(argv[1]) : 1000);
    int viewPlaneResolutionY = (argc > 2 ? std::stoi(argv[2]) : 1000);
    ...
    tasks.reserve(viewPlaneResolutionX);
    
    ThreadPool pool(16);

    for (int x = 0; x < viewPlaneResolutionX; x++)
    {
        auto task = [&image, &scene, &viewPlane, viewPlaneResolutionY, x, numOfSamples]()
        {
        ...
        tasks.push_back(pool.Enqueue(task));
    }
